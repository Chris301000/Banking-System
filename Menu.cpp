...
void Menu::Delete_person_display(People people)
{
...
void Menu::Make_Account_Display(People people)
{
...
void Menu::Person_account_Display(People people)
{
    std::string user_given_name;
    People_List_Display(people);
    ...
    if(people.findPerson(user_given_name))
    {
        Person person_to_display_account = people.returnAtName(user_given_name);
        if(person_to_display_account.HaveAccount())
        {
            Account person_account = person_to_display_account.getAccount();
            std::cout << "Name: " << person_to_display_account.getName() << std::endl;
        ...
        {
            std::cout << "Unable to display the account since there is no account made for this person." << std::endl;
        }
    }
};
...
void Menu::People_with_account_Display(People people)
{
    std::cout << "People with Account List: " << std::endl;
    int number_of_people = people.numOfPeople();
    for(int i = 0; i < number_of_people; i++)
    {
        if(people.returnAtIndex(i).HaveAccount())
        {
            Person_info_Display(people, people.returnAtIndex(i).getName());
        }
    }
};
